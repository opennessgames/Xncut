 ...
 * @LastEditors: xixi_
 * @LastEditTime: 2025-10-14 20:00:20
 * @FilePath: /XncutPenTool/XncutPenTool/XncutPenToolCoreWidget/XncutPenToolWidget3.cpp
 * Copyright (c) 2020-2025 by xixi_ , All Rights Reserved.
 */

#include "XncutPenToolWidget3.h"
#include <QMouseEvent>
#include <QPainter>
...
XncutPenToolWidget3::XncutPenToolWidget3(QWidget *Parent)
    : QWidget(Parent)
{
    /* 开启一些属性 */
    /* 获取焦点 */
...
XncutPenToolWidget3::~XncutPenToolWidget3()
{
}

void XncutPenToolWidget3::ClearAll()
{
    /* 清空所有的点 */
    M_AllPoint.clear();
    M_AllPointNumber = -1;
...
QJsonDocument XncutPenToolWidget3::GetJsonDocument()
{
    /* 根JSON对象 */
...
XncutPenToolWidget3::XncutPenPoint XncutPenToolWidget3::CreatePenAnchorPoint(const QPoint &Point)
{
...
bool XncutPenToolWidget3::IsNearPoint(QPoint PointA, QPoint PointB, int Radius)
{
...
void XncutPenToolWidget3::GetPositionType(QPoint Point)
{
    /**
     *  @brief 检查点
     **/
...
QPoint XncutPenToolWidget3::CubicBezierPoint(const QPoint &P0, const QPoint &P1, const QPoint &P2, const QPoint &P3, double t)
{
    /* 缓存量 */
    double u = 1 - t;
    double tt = t * t;
...
bool XncutPenToolWidget3::GetClosestOnCubicBezier(QPoint *DestPoint, const QPoint &P0, const QPoint &P1, const QPoint &P2, const QPoint &P3, const QPoint &P, int Steps)
{
...
QPoint XncutPenToolWidget3::Lerp(const QPoint &P1, const QPoint &P2, double t)
{
    return QPoint(
        P1.x() + (P2.x() - P1.x()) * t,
        P1.y() + (P2.y() - P1.y()) * t);
...
void XncutPenToolWidget3::SplitCubicBezier(const QPoint &P0, const QPoint &P1, const QPoint &P2, const QPoint &P3, double t)
{
    /* 第一次线性插值 */
...
void XncutPenToolWidget3::DrawCubicBezier(QPainter *Painter, QPoint StartPoint, QPoint C1, QPoint C2, QPoint EndPoint)
{
...
void XncutPenToolWidget3::DrawAnchorPoint(QPainter *Painter, XncutPenPoint AnchorPoint, bool Active)
{
...
void XncutPenToolWidget3::DrawSquare(QPainter *Painter, const QPoint Point, int R, int LineWidth, const QColor FillColor, const QColor EdgeColor)
{
    Painter->save(); /* Save the current painter state */
...
void XncutPenToolWidget3::DrawControlPoint(QPainter *Painter, QPoint ControlPoint, QPoint Point, bool Active)
{
    Painter->save(); /* Save the current painter state */
 ...
 **/
void XncutPenToolWidget3::mousePressEvent(QMouseEvent *Event)
{
    /* 只有左键处理业务逻辑 */
...
void XncutPenToolWidget3::mouseReleaseEvent(QMouseEvent *Event)
{
    M_MouseIsPress = false;
}

void XncutPenToolWidget3::mouseDoubleClickEvent(QMouseEvent *Event)
{
    /* 避免在未闭合时删除点 */
    if (!M_IsColse)
    {
 ...
 * TODO: 还需优化
 **/
void XncutPenToolWidget3::mouseMoveEvent(QMouseEvent *Event)
{
    /* 判空保护 */
    if (!M_CurrentSelectedPoint)
    {
...
void XncutPenToolWidget3::keyPressEvent(QKeyEvent *Event)
{
    M_AltIsPress = (Event->key() == Qt::Key_Alt);
}

void XncutPenToolWidget3::keyReleaseEvent(QKeyEvent *Event)
{
    switch (Event->key())
    {
    case Qt::Key_Alt:
...
void XncutPenToolWidget3::paintEvent(QPaintEvent *Event)
{
    /* 提前判空 */
    ...
    for (register_t i = 0; i < M_AllPointNumber; i++)
    {
        ...
        /* 末尾与头部建立链接 */
        if (i == M_AllPointNumber - 1 && M_IsColse)
        {
            /* 获取第一个点 */
            XncutPenPoint FirstPenPoint = M_AllPoint.at(0);

            /* 首尾连接 */
            DrawCubicBezier(&XncutPenToolWidgetPainter, Point.Point, Point.ControlInPoint, FirstPenPoint.Point, FirstPenPoint.Point);
        }
